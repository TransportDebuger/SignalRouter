...
/**
@class SignalRouter
@brief Обеспечивает потокобезопасную регистрацию и асинхронную диспетчеризацию
сигналов.
*/
class SignalRouter {
 public:
  ...
  /**
    @brief Конструирует экземпляр маршрутизатора и инициализирует системные
    ресурсы.
    @param[in] sys_calls Указатель на интерфейс системных вызовов. Если равен
    nullptr, автоматически инстанцируется нативная реализация.
    @throw std::system_error При ошибке сохранения маски сигналов или создания
    файлового дескриптора signalfd.
    */
  explicit SignalRouter(std::unique_ptr<ISystemCalls> sys_calls = nullptr);

  /// @brief Деструктор. Гарантирует остановку фонового потока, закрытие
  /// дескрипторов и восстановление исходной маски сигналов.
  ~SignalRouter();
  ...
  /**
    ...
    @throw std::invalid_argument Если номер сигнала недопустим (SIGKILL,
    SIGSTOP, <= 0, >= NSIG).
    @throw std::system_error При ошибке системного вызова sigprocmask или
    signalfd.
    */
  ...
  /**
    @brief Запускает фоновый поток мониторинга событий и диспетчеризации
    сигналов.
  ...
  /**
    @brief Возвращает ссылку на глобальный экземпляр маршрутизатора (Meyers
    Singleton).
    @return SignalRouter& Ссылка на единственный экземпляр маршрутизатора в
    процессе.
    */
  static SignalRouter& Global();

 private:
  /// @private Предварительное объявление структуры скрытой реализации (PIMPL)
  /// для инкапсуляции POSIX-специфичных типов.
  class Impl;

  /// @private Умный указатель на скрытую реализацию, обеспечивающий строгую
  /// инкапсуляцию состояния и стабильность ABI.
